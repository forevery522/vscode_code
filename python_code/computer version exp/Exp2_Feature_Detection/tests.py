...
import features_homework as features
import traceback
...
def compare_match(match1, match2):
    if not np.isclose(match1.distance, match2.distance, rtol=1e-3, atol=1e-5): return False
    if not np.isclose(match1.queryIdx, match2.queryIdx, rtol=1e-3, atol=1e-5): return False
    if not np.isclose(match1.trainIdx, match2.trainIdx, rtol=1e-3, atol=1e-5): return False
    return True
...
#Test for Todo7 and Todo 8
HKD1 = features.HarrisKeypointDetector()
SSDFM = features.SSDFeatureMatcher()
Ratio = features.RatioFeatureMatcher()

image1 = np.array(Image.open('resources/triangle1.jpg'))
image2 = np.array(Image.open('resources/triangle2.jpg'))

point1 = HKD1.detectKeypoints(image1)
point2 = HKD1.detectKeypoints(image2)

desc1 = MFD.describeFeatures(image1, point1)
desc2 = MFD.describeFeatures(image2, point2)

matches_ssd = SSDFM.matchFeatures(desc1, desc2)
matches_ra = Ratio.matchFeatures(desc1, desc2)

try_this(7, SSDFM.matchFeatures, matches_ssd, compare_match, desc1, desc2)
try_this(8, Ratio.matchFeatures, matches_ra, compare_match, desc1, desc2)